...
import { syncActorToSite } from "@/lib/actor-sync";
import { can } from "@/lib/permissions";

export const maxDuration = 600;
const inputSchema = z.object({ siteId: z.string().cuid(), actorIds: z.array(z.string().cuid()).min(1).max(5) }).strict();
export async function POST(req: Request) {
  try {
    ...
    const { siteId, actorIds } = inputSchema.parse(await req.json());
    ...
    for (const actorId of ids) {
      try { results.push(await syncActorToSite(actorId, siteId, user)); }
