...
const mocks = vi.hoisted(() => ({ auth: vi.fn(), read: vi.fn(), upsert: vi.fn(), encrypt: vi.fn(), audit: vi.fn() }));
vi.mock("@/lib/authz", () => ({ requireAdmin: mocks.auth }));
vi.mock("@/lib/content-ai", () => ({ readAiConfig: mocks.read }));
vi.mock("@/lib/prisma", () => ({ prisma: { contentAiConfig: { upsert: mocks.upsert } } }));
vi.mock("@/lib/crypto", () => ({ encrypt: mocks.encrypt, decrypt: vi.fn() }));
vi.mock("@/lib/audit", () => ({ logAudit: mocks.audit }));
import { GET, PUT } from "./route";
const config = { enabled: true, model: "gpt-4.1-mini", apiKeyEnc: "encrypted", apiKeyIv: "iv", apiKeyTag: "tag" };
...
describe("content AI settings", () => {
  ...
  it("only returns public settings and a key-presence flag", async () => {
    ...
    expect(await response.json()).toEqual({ enabled: true, model: "gpt-4.1-mini", hasApiKey: true });
  ...
  it("retains the saved encrypted key when only settings change", async () => {
    ...
    expect(mocks.upsert).toHaveBeenCalledWith(expect.objectContaining({ update: { ...config, enabled: false } }));
    expect(mocks.encrypt).not.toHaveBeenCalled();
  });
