...
export function ComicSitesManager({ initialSites, role }: { initialSites: ComicSiteRow[]; role: Role }) {
  ...
  function addSite(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        const site = await apiFetch<ComicSiteRow>("/api/comic-sites", {
          method: "POST",
          body: JSON.stringify({
            name: form.name,
  ...
  function pingSite(id: string) {
    startTransition(async () => {
      try {
        const { healthStatus } = await apiFetch<{ healthStatus: ComicSiteRow["healthStatus"] }>(`/api/comic-sites/${id}/ping`, { method: "POST" });
  ...
  function toggleActive(site: ComicSiteRow) {
    startTransition(async () => {
      try {
        const updated = await apiFetch<ComicSiteRow>(`/api/comic-sites/${site.id}`, {
          method: "PATCH",
  ...
  function toggleComicType(site: ComicSiteRow, type: "MANGA" | "DOUJIN") {
    ...
    startTransition(async () => {
      try {
        const updated = await apiFetch<ComicSiteRow>(`/api/comic-sites/${site.id}`, {
          method: "PATCH",
          body: JSON.stringify({ comicTypes: next }),
        });
        setSites((prev) => prev.map((s) => (s.id === site.id ? updated : s)));
  ...
  function deleteSite(id: string) {
    if (!confirm("ลบเว็บนี้?")) return;
    startTransition(async () => {
      try {
        await apiFetch(`/api/comic-sites/${id}`, { method: "DELETE" });
    ...
    <div className="ad-grid">
      <div className="panel">
        <div className="panel-head">
          <h3>เว็บที่เชื่อมต่อ (Doujin/Comic)</h3>
            ...
            {canManage && (
              <div style={{ padding: "0 12px 12px", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                {(["DOUJIN", "MANGA"] as const).map((type) => {
                  ...
                  return (
                    <button key={type} type="button" className={`chip ${active ? "active" : ""}`} disabled={pending} onClick={() => toggleComicType(s, type)}>
          ...
          <div className="panel">
            ...
            <form onSubmit={addSite}>
              ...
              <div className="field">
                ...
                <input type="url" value={form.baseUrl} onChange={(e) => setForm({ ...form, baseUrl: e.target.value })} placeholder="https://doujin-site-a.com" />
              </div>
              <div className="field">
