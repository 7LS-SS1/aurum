...
export function ComicTagsManager({ initialTags, initialPagination }: { initialTags: ComicTagRow[]; initialPagination: PaginationState }) {
  ...
  async function fetchPage(page: number, query = q) {
    ...
    const res = await apiFetch<{ tags: ComicTagRow[]; pagination: PaginationState }>(`/api/comic-tags?${params.toString()}`);
    setTags(res.tags);
  ...
  function startEdit(t: ComicTagRow) {
    setEditingId(t.id);
  ...
  function saveEdit() {
    ...
    startTransition(async () => {
      try {
        const updated = await apiFetch<{ id: string; name: string }>(`/api/comic-tags/${editingId}`, { method: "PATCH", body: JSON.stringify({ name }) });
        setTags((prev) => prev.map((t) => (t.id === editingId ? { ...t, name: updated.name } : t)));
        setEditingId(null);
        notify("บันทึกแล้ว — อัปเดตทุกคอมมิคที่ใช้แท็กนี้แล้ว");
      } catch (err) {
  ...
  function deleteTag(id: string, name: string, comicCount: number) {
    const extra = comicCount > 0 ? ` (ใช้อยู่ใน ${comicCount} คอมมิค)` : "";
    if (!confirm(`ลบแท็ก "${name}"${extra}? แท็กนี้จะถูกลบออกจากทุกคอมมิคที่ใช้อยู่`)) return;
    startTransition(async () => {
      try {
        await apiFetch(`/api/comic-tags/${id}`, { method: "DELETE" });
  ...
  return (
    <div className="panel">
        ...
        <div key={t.id} className="site-row" style={{ cursor: "default" }}>
          {editingId === t.id ? (
            ...
            <div className="site-info">
              ...
              <div className="url">ใช้ใน {t.comicCount.toLocaleString("th-TH")} คอมมิค</div>
            </div>
          )}
          {editingId === t.id ? (
            <>
            ...
            <>
              ...
              <button
                ...
                disabled={pending}
                onClick={() => deleteTag(t.id, t.name, t.comicCount)}
              >
                ลบ
              </button>
            </>
      ...
      {pagination.totalPages > 1 && (
        <nav className="pagination admin-pagination" aria-label="Comic tags pagination">
