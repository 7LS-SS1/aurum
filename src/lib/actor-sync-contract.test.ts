...
describe("actor sync destination contract", () => {
  it("creates a new actor and verifies returned data", async () => {
    const fetch = vi.fn().mockResolvedValueOnce(response(null)).mockResolvedValueOnce(response(remote()));
  ...
  it("repeated pushes skip writes and never upload unchanged images", async () => {
    const fetch = vi.fn().mockImplementation(() => Promise.resolve(response(remote())));
    vi.stubGlobal("fetch", fetch);
