...
echo 'ACTOR_TEST_BOOT_START' . PHP_EOL;
require getenv( 'WP_LOAD_PATH' );
echo 'ACTOR_TEST_BOOT_OK' . PHP_EOL;
if ( ! function_exists( 'aurum_actor_sync_register' ) ) { require_once dirname( __DIR__ ) . '/includes/actor-sync.php'; }
aurum_actor_sync_register();
aurum_actor_sync_taxonomy_register();
...
function actor_test_request( $method, $id, $payload = null ) {
 echo "ACTOR_TEST_REQUEST $method $id\n";
...
try {
 ...
 actor_test_expect( 200 === $read->get_status() && array( 'found' => false ) === $read->get_data(), 'missing read uses explicit JSON' );
 ...
 for ( $i = 0; $i < 3; $i++ ) {
  ...
  actor_test_expect( $id === $data['remoteId'] && $term_id === (int) $data['termId'] && 'skipped' === $data['status'], 'repeat keeps post and term IDs' );
 }
 $payload['name'] = 'ชื่อใหม่';
 $data = actor_test_request( 'PUT', $external, $payload )->get_data();
 actor_test_expect( $id === $data['remoteId'] && 'updated' === $data['status'], 'rename keeps ID' );
 actor_test_expect( 'ชื่อใหม่' === get_term( $term_id, 'aurum_video_actor' )->name, 'term rename keeps term ID' );
 ...
 actor_test_expect( '' === get_term_meta( $term_id, '_av123_actor_profile_image_url', true ), 'term image URL removal' );
