...
function aurum_render_video_player( $post_id ) {
	...
	} elseif ( ! empty( $meta['video_url'] ) ) {
		$poster_attr = ! empty( $meta['thumbnail_url'] ) ? ' poster="' . esc_url( $meta['thumbnail_url'] ) . '"' : '';
		printf(
			'<video id="aurum-video-%d" controls playsinline preload="metadata"%s data-src="%s"></video>',
			(int) $post_id,
			$poster_attr,
			esc_url( $meta['video_url'] )
		);
	} else {
		echo '<div class="aurum-video-missing">' . esc_html__( 'ยังไม่มีวิดีโอสำหรับเรื่องนี้', 'aurum-video' ) . '</div>';
	}
